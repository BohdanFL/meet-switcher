...
export class TileBadgeDecorator {
    ...
    public cleanTileName(rawText: string): {
        name: string;
        isPresentation: boolean;
    } {
        if (!rawText) return { name: '', isPresentation: false };
        const presentationRegex =
            /(?:\(presentation\)|\(презентація\)|\(презентация\))/i;
    ...
    public decorateTileNameElement(
        nameEl: HTMLElement,
        ...
        const storedIsPres =
            nameEl.getAttribute('data-ms-is-presentation') === 'true';
        ...
        const originalName =
            storedOriginal || overrideOriginalName || cleanFromRaw.name;
        ...
        if (alias) {
            ...
            const hasFormattedAttr =
                nameEl.getAttribute('data-ms-formatted') === combinedText;
            ...
            if (!hasAliasSpan || !hasFormattedAttr) {
                ...
                if (
                    tooltipContainer &&
                    typeof tooltipContainer.querySelector === 'function'
                ) {
                    const tooltipEl =
                        tooltipContainer.querySelector<HTMLElement>(
                            '[role="tooltip"]',
                        );
                    if (tooltipEl) {
                ...
                if (
                    !(nameEl as any)._hasMsDblClick &&
                    typeof nameEl.addEventListener === 'function'
                ) {
                    ...
                    nameEl.addEventListener('dblclick', async (e: any) => {
                        ...
                        if (win && typeof win.prompt === 'function') {
                            ...
                            if (newAlias !== null) {
                                if (newAlias.trim()) {
                                    await this.aliasManager.setAlias(
                                        originalName,
                                        newAlias.trim(),
                                    );
                                } else {
                                    await this.aliasManager.removeAlias(
                                        originalName,
                                    );
                                }
                            }
    ...
    public findTileNameElement(tile: HTMLElement): HTMLElement | null {
        ...
        const xeazbcSpan = tile.querySelector<HTMLElement>(
            '.XEazBc span.notranslate, .XEazBc span',
        );
        if (xeazbcSpan && !this.isIconElement(xeazbcSpan)) return xeazbcSpan;

        // 2. Standard Google Meet name element: span.zWGUib
        const zwguib = tile.querySelector<HTMLElement>(
            'span.zWGUib, div.zWGUib',
        );
        ...
        if (typeof tile.querySelectorAll === 'function') {
            const candidates = Array.from(
                tile.querySelectorAll<HTMLElement>(
                    '.notranslate, [data-self-name], span[title]',
                ),
            );
            for (const el of candidates) {
            ...
            const fallback = tile.querySelector<HTMLElement>(
                '.notranslate, [data-self-name], span[title]',
            );
    ...
    public updateTile(share: ScreenShare): void {
        if (
            !share.tileElement ||
            typeof share.tileElement.querySelector !== 'function'
        ) {
            return;
        }
        ...
        const existingBadge = share.tileElement.querySelector<HTMLElement>(
            `.${BADGE_CLASS}`,
        );
    ...
    public destroy(): void {
        if (typeof document !== 'undefined') {
            ...
            const modified =
                document.querySelectorAll<HTMLElement>('[data-ms-original]');
            modified.forEach((el) => {
                if (el.tagName === 'I') {
